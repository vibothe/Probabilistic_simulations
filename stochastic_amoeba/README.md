...
## Ì≥å Problem Description

We simulate the evolution of an amoeba population where each amoeba independently undergoes one of four actions at each discrete time step:

- **Die** with probability `p_die`
- **Stay the same** (reproduce 1) with probability `p_stay`
- **Divide into 2** (reproduce 2) with probability `p_double`
- **Divide into 3** (reproduce 3) with probability `p_triple`

The simulation starts with a single amoeba and proceeds step by step. If the population ever drops to zero, extinction occurs. If it survives for a maximum number of steps or grows beyond a specified threshold, the simulation halts early.
